    ...
    const bool writeIterations)
{
    ...
    for (int k = 0;; k++)
    {
        ...
        timerCombinedLine6.Restart();
        float sigma = CombinedLine6(p, z);
        timerCombinedLine6.Pause();
        ...
        timerCombinedSaxpyLine16.Restart();
        CombinedSaxpyLine16(x, p, r, alpha, beta);
        timerCombinedSaxpyLine16.Pause();
        if (writeIterations)
            WriteAsImage("x", x, k, 0, 127);
    }
}
